...
async function resolveVisitor(id) {
  if (!isValidUuid(id)) {
    return { kind: 'guest', profile: null };
  }
  try {
    ...
    if (!userErr && userProfile) {
      return { kind: 'user', profile: userProfile };
    }

    const { data: partnerProfile, error: partnerErr } = await supabase
      .from('partner_profiles')
      .select('id, partner_data')
      .eq('id', id)
      .maybeSingle();

    if (!partnerErr && partnerProfile) {
      return { kind: 'partner', profile: partnerProfile };
    }
...
function showAuthGate() {
  ...
  const dash = document.getElementById('referralDashboard');
...
function renderProfileHeader(profile) {
  ...
  const name = ud.full_name || 'Valued Student';
  const email = ud.email || '';
  const initials = name.trim().split(/\s+/).map(w => w[0]).join('').slice(0, 2).toUpperCase() || 'ID';
  const refCode = ud.referral_code || '';
  host.innerHTML = `
    <div class="ph-card">
      <div class="ph-avatar">${escapeHtml(initials)}</div>
      <div class="ph-info">
        <span class="ph-greeting"><i class="fa-solid fa-hand-sparkles"></i> Welcome back,</span>
        <h2 class="ph-name">${escapeHtml(name)}</h2>
        ${email ? `<span class="ph-email"><i class="fa-solid fa-envelope"></i> ${escapeHtml(email)}</span>` : ''}
        ${refCode ? `<span class="ph-code"><i class="fa-solid fa-id-badge"></i> Code: ${escapeHtml(refCode)}</span>` : ''}
      </div>
      <div class="ph-verified"><i class="fa-solid fa-circle-check"></i> Verified</div>
    </div>
  `;
}
...
async function loadReferralData() {
  ...
  try {
    ...
    const result = await resolveVisitor(sessionUser && sessionUser.id ? sessionUser.id : null);

    if (result.kind === 'partner') {
      hideLoading();
      showToast('info', 'Partner Account', 'This is a partner account. Redirecting you to the partner dashboard...');
      setTimeout(() => { window.location.href = 'partner.html'; }, 1500);
      return;
    }
...
function initMenu() {
  ...
  $('menuLogout').addEventListener('click', () => {
    localStorage.removeItem('idt_user');
    showToast('info', 'Logged Out', 'You have been logged out. Redirecting to referral page...');
    setTimeout(() => { showAuthGate(); window.scrollTo(0, 0); }, 800);
  });
}
