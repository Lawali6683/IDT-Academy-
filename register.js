...
function showLoading() {
  ...
  if (!loader) {
    const loaderHTML = `
      ...
      <style>
        .idt-loader-2{position:fixed;inset:0;z-index:99999;background:#05060f;display:flex;align-items:center;justify-content:center;font-family:'Segoe UI',Roboto,Helvetica,Arial,sans-serif;transition:opacity .45s ease,visibility .45s ease;overflow:hidden;user-select:none}
        ...
        .i2-b3{width:320px;height:320px;left:32%;bottom:-150px;background:#f59e0b;opacity:.3;animation:i2drift3 20s ease-in-out infinite}
        .i2-glow{position:absolute;left:50%;top:50%;width:520px;height:520px;margin:-260px 0 0 -260px;background:radial-gradient(circle,rgba(139,92,246,.18),transparent 65%)}
        .i2-grid{position:absolute;inset:0;background-image:linear-gradient(rgba(148,163,184,.06) 1px,transparent 1px),linear-gradient(90deg,rgba(148,163,184,.06) 1px,transparent 1px);background-size:44px 44px}
        ...
        .i2-loadbar span{display:block;height:100%;width:100%;border-radius:4px;background:linear-gradient(90deg,#7c3aed,#22d3ee,#fbbf24);transform-origin:left;transition:transform .15s linear}
        ...
        @keyframes i2drift1{0%,100%{transform:translate(0,0)}50%{transform:translate(60px,40px)}}
        @keyframes i2drift2{0%,100%{transform:translate(0,0)}50%{transform:translate(-50px,30px)}}
        @keyframes i2drift3{0%,100%{transform:translate(0,0)}50%{transform:translate(40px,-40px)}}
      </style>
    `;
  ...
  loaderInterval = setInterval(() => {
    loaderPercent = Math.min(loaderPercent + 4, 90);
    const n = document.getElementById('i2num');
    if (n) n.textContent = loaderPercent;
  }, 40);
}
...
function getUrlRef() {
  ...
  if (!ref) {
    ...
    if (!m) m = path.match(/\/index\/ref\/([A-Za-z0-9]{4,8})/i);
    if (m) ref = m[1];
  }
...
function friendlyLoginError(json) {
  const raw = String((json && (json.error || json.message)) || '').toLowerCase();
  if (raw.includes('invalid login credentials') || raw.includes('invalid_credentials')) return 'Wrong email or password. Please check and try again.';
  if (raw.includes('no account') || raw.includes('not found') || raw.includes('register')) return 'No account found with this email. Please use the "Register" tab to create one.';
  if (raw.includes('password')) return 'Wrong password. Please try again.';
  if (raw.includes('network') || raw.includes('fetch')) return 'Network problem. Check your internet and try again.';
  return 'Login failed. Please try again or contact support.';
}
function extractCourse(row) {
  let c = row;
  if (c && typeof c === 'object' && c.course_data && typeof c.course_data === 'object') {
    c = c.course_data;
  }
  if (!c || typeof c !== 'object') return null;
  const id = row && row.id ? String(row.id) : (c.id ? String(c.id) : '');
  if (!id) return null;
  return {
    id: id,
    course_name: c.course_name || c.name || 'Unnamed Course',
...
async function loadCourses() {
  ...
  try {
    ...
    list.forEach((c) => {
      ...
      opt.value = c.id;
      opt.textContent = c.course_name + ' - \u20A6' + c.price.toLocaleString('en-NG');
    ...
    if (urlCourseId && !courseMap[urlCourseId]) {
      ...
      courseMap[fake.id] = fake;
      const opt = document.createElement('option');
      opt.value = fake.id;
      opt.textContent = fake.course_name + ' - \u20A6' + fake.price.toLocaleString('en-NG');
      opt.selected = true;
      sel.appendChild(opt);
    }
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    ...
    if (localUser && sessionData && sessionData.session) {
  ...
  if (ref && $('regRef')) {
    ...
    sessionStorage.setItem('idt_ref', ref);
  }
